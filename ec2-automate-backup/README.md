...
* run ec2-automate-backup to snapshot all EBS volumes that contain the tag "Backup=true" (example: ``"0 0 * * 0 ec2-user /home/ec2-user/ec2-automate-backup.sh -s tag -t "Backup,Values=true" > /home/ec2-user/ec2-automate-backup_`date +"%Y%m%d"`.log"``)
...
* To backup a selected group of EBS volumes on a daily schedule tag each volume you wish to backup with the tag "Backup=true" and run ec2-automate-backup using cron as follows: `0 0 * * * ec2-automate-backup -s tag -t "Backup,Values=true"`
