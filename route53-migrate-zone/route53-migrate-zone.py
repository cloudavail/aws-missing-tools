...
parser.add_argument('--log-level', dest='loglevel',
                    help=str('set the log level when running {app_name!s}.'.format(app_name=app_name)),
                    default='WARNING', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'])
parser.add_argument('--config',
                    help=str('choose the configuration file to be used when running {app_name!s}'
...
source_connection = boto.route53.Route53Connection(aws_access_key_id=source_zone_access_key,
                                                   aws_secret_access_key=source_zone_secret_key)
destination_connection = boto.route53.Route53Connection(aws_access_key_id=destination_zone_access_key,
                                                        aws_secret_access_key=destination_zone_secret_key)
...
try:
    ...
    logging.critical('An error occured when attempting to create a connection to AWS.')
    logging.critical('The error message given was: {error!s}.'.format
...
for record in source_zone_records:
    print dir(record)
    print record.name
    print record.alias_dns_name
    print record.alias_hosted_zone_id
...
for record in resource_record_dict:
    ...
    # if there are 99 uncomitted change elements than they must be committed -
    # Amazon only accepts up to 99 change elements at a given time
    # if the number of examined records is equal to the number of records then
    # we can commit as well - we are now done examing records
    if uncommitted_change_elements >= 99 or examined_record_count == len(resource_record_dict):
        ...
        destination_zone_record_changeset = boto.route53.record.ResourceRecordSets(destination_connection,
                                                                                   destination_zone_id)
